...
# -*- coding: utf-8 -*-
"""
Desktop App với tính năng lọc theo vùng miền
"""
...
class RegionalVoiceComparisonApp:
    def __init__(self, root):
        self.root = root
        self.root.title("ViSpeech - Regional Voice Comparison")
        self.root.geometry("1100x750")
        self.root.configure(bg='#f0f0f0')
        
        # Khởi tạo VoiceComparisonKNN
        self.voice_comparison = None
        self.selected_regions = ['North', 'Central', 'South']  # Mặc định tất cả vùng
    ...
    def load_voice_comparison(self):
        ...
        try:
            self.voice_comparison = RegionalVoiceComparisonKNN(self.selected_regions)
            if self.voice_comparison.is_ready:
                print(f"RegionalVoiceComparisonKNN loaded successfully for regions: {self.selected_regions}")
            else:
                print("RegionalVoiceComparisonKNN not ready - training required")
        except Exception as e:
            print(f"Error loading RegionalVoiceComparisonKNN: {e}")
            self.voice_comparison = None
    ...
    def create_widgets(self):
        ...
        title_label = tk.Label(header_frame, text="ViSpeech - Regional Voice Comparison", 
                              font=('Arial', 16, 'bold'), fg='white', bg='#667eea')
        title_label.pack(expand=True)
        
        subtitle_label = tk.Label(header_frame, text="So sanh va tim kiem giong noi theo vung mien", 
        ...
        # Regional selection
        region_frame = tk.LabelFrame(main_frame, text="Chon Vung Mien", 
                                    font=('Arial', 12, 'bold'), bg='#f0f0f0')
        region_frame.pack(fill='x', pady=10)
        
        # Checkboxes for regions
        checkbox_frame = tk.Frame(region_frame, bg='#f0f0f0')
        ...
        for i, region in enumerate(regions):
            ...
            cb = tk.Checkbutton(checkbox_frame, text=region, variable=var,
                               command=self.on_region_change, font=('Arial', 10))
    ...
    def on_region_change(self):
        """Xử lý khi thay đổi vùng miền"""
        selected_regions = [region for region, var in self.region_vars.items() if var.get()]
        
        if not selected_regions:
            messagebox.showwarning("Canh bao", "Phai chon it nhat 1 vung mien!")
            # Reset về trạng thái trước đó
            for region, var in self.region_vars.items():
                var.set(region in self.selected_regions)
            return
        
        self.selected_regions = selected_regions
        print(f"Vung mien duoc chon: {self.selected_regions}")
        
        # Reload voice comparison với vùng miền mới
        self.load_voice_comparison()
        self.check_system_status()
    ...
    def check_system_status(self):
        ...
        region_info = f"Vung mien: {', '.join(self.selected_regions)}"
        self.status_var.set(f"He thong san sang - {region_info}")
    
    def run_training(self):
        """Chạy training dữ liệu theo vùng miền"""
        try:
            self.progress_var.set("Dang training du lieu theo vung mien...")
    ...
    def _training_thread(self):
        ...
        try:
            # Import và chạy regional training
            from train_regional import RegionalAudioTrainer
            
            trainer = RegionalAudioTrainer()
            success = trainer.train_regional_data(self.selected_regions)
            ...
            if success:
                ...
                self.root.after(0, lambda: messagebox.showinfo("Thanh cong", f"Training hoan thanh cho vung mien: {', '.join(self.selected_regions)}"))
            else:
                self.root.after(0, lambda: self.progress_bar.stop())
                self.root.after(0, lambda: self.progress_var.set("Training that bai"))
                self.root.after(0, lambda: messagebox.showerror("Loi", "Training that bai!"))
    ...
    def _display_audio_features(self):
        ...
        features_text += f"Vung mien: {', '.join(self.selected_regions)}\n"
...
class RegionalVoiceComparisonKNN:
    def __init__(self, regions=None):
        self.regions = regions or ['North', 'Central', 'South']
        self.super_metadata_file = self._get_regional_file()
    ...
    def _get_regional_file(self):
        """Lấy file metadata theo vùng miền"""
        region_name = "_".join(self.regions).lower()
        return f"super_metadata/regional/regional_{region_name}.csv"
    ...
    def load_data(self):
        ...
        try:
            ...
            if os.path.exists(self.super_metadata_file):
                ...
                print(f"Da load {len(self.df)} records tu regional metadata: {self.super_metadata_file}")
            else:
                print(f"Khong tim thay regional metadata file: {self.super_metadata_file}")
                return
    ...
    def prepare_features(self):
        ...
        try:
            ...
            self.is_ready = True
            print(f"Da train K-NN model voi {len(self.feature_columns)} features cho vung mien: {self.regions}")
...
def main():
    """Hàm chính"""
    print("ViSpeech - Regional Voice Comparison Desktop App")
    print("=" * 60)
    
    root = tk.Tk()
    app = RegionalVoiceComparisonApp(root)
