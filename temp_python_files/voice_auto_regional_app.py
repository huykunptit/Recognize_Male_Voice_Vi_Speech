...
# -*- coding: utf-8 -*-
"""
Desktop App với tính năng phân loại vùng miền tự động
"""
...
class AutoRegionalVoiceApp:
    def __init__(self, root):
        self.root = root
        self.root.title("ViSpeech - Auto Regional Voice Detection")
    ...
    def load_voice_comparison(self):
        """Load VoiceComparisonKNN và Region Classifier"""
        try:
            self.voice_comparison = AutoRegionalVoiceComparisonKNN()
            if self.voice_comparison.is_ready:
                print("AutoRegionalVoiceComparisonKNN loaded successfully")
            else:
                print("AutoRegionalVoiceComparisonKNN not ready - training required")
        except Exception as e:
            print(f"Error loading AutoRegionalVoiceComparisonKNN: {e}")
            self.voice_comparison = None
    ...
    def create_widgets(self):
        ...
        title_label = tk.Label(header_frame, text="ViSpeech - Auto Regional Voice Detection", 
                              font=('Arial', 16, 'bold'), fg='white', bg='#667eea')
        title_label.pack(expand=True)
        
        subtitle_label = tk.Label(header_frame, text="Tu dong phat hien vung mien va so sanh giong noi", 
        ...
        region_label = tk.Label(region_frame, textvariable=self.region_result_var, 
        ...
        compare_btn = tk.Button(control_frame, text="Phat Hien & So Sanh", 
                               command=self.detect_and_compare, bg='#28a745', fg='white',
        ...
        features_frame = tk.LabelFrame(main_frame, text="Thuoc tinh Audio & Ket Qua Phat Hien", 
                                      font=('Arial', 12, 'bold'), bg='#f0f0f0')
        features_frame.pack(fill='x', pady=10)
        
        self.features_text = tk.Text(features_frame, height=8, font=('Courier', 9), 
        ...
        # Results
        results_frame = tk.LabelFrame(main_frame, text="Ket qua So Sanh", 
                                     font=('Arial', 12, 'bold'), bg='#f0f0f0')
    ...
    def check_system_status(self):
        ...
        self.status_var.set("He thong san sang - Tu dong phat hien vung mien")
    ...
    def detect_and_compare(self):
        """Phát hiện vùng miền và so sánh giọng nói"""
        ...
        thread = threading.Thread(target=self._detect_and_compare_thread)
        thread.daemon = True
        thread.start()
    
    def _detect_and_compare_thread(self):
        """Thread để phát hiện vùng miền và so sánh giọng nói"""
        try:
            # Cập nhật UI
            ...
            # Bước 4: Hiển thị thuộc tính và kết quả phát hiện
            self.root.after(0, lambda: self._display_audio_features(region_result))
            
            # Bước 5: So sánh với dữ liệu từ vùng miền được phát hiện
            self.root.after(0, lambda: self.progress_var.set("Dang so sanh..."))
            results = self.voice_comparison.find_similar_voices(audio_path, k=10, features=features, target_region=self.detected_region)
            ...
            error_msg = f"Loi khi phat hien va so sanh: {str(e)}"
            self.root.after(0, lambda: messagebox.showerror("Loi", error_msg))
        finally:
            # Dừng progress bar
            self.root.after(0, lambda: self.progress_bar.stop())
    ...
    def _save_features_to_json(self, features, region_result):
        ...
        try:
            ...
            print(f"Da luu thuoc tinh va ket qua phat hien vao: {self.features_json_file}")
    ...
    def _update_results(self, results):
        ...
        region_info = f"Vung mien phat hien: {self.detected_region}" if self.detected_region else "Chua phat hien vung mien"
        self.status_var.set(f"Tim thay {len(results)} ket qua tuong tu - {region_info}")
    
    def _display_audio_features(self, region_result):
        """Hiển thị thuộc tính audio và kết quả phát hiện"""
        ...
        features_text = "THUOC TINH AUDIO & KET QUA PHAT HIEN:\n"
        features_text += "=" * 60 + "\n\n"
        ...
        # Hiển thị kết quả phát hiện vùng miền
        if region_result:
            features_text += "KET QUA PHAT HIEN VUNG MIEN:\n"
            features_text += "-" * 40 + "\n"
            features_text += f"Vung mien du doan: {region_result['predicted_region']}\n"
            features_text += f"Do tin cay: {region_result['confidence']:.2f}%\n"
            features_text += f"Xac suat cac vung:\n"
            for region, prob in region_result['probabilities'].items():
                features_text += f"  - {region}: {prob:.2f}%\n"
            features_text += "\n"
...
class AutoRegionalVoiceComparisonKNN:
    ...
    def load_data(self):
        ...
        try:
            ...
            # Chuẩn bị features cho K-NN và Region Classifier
    ...
    def prepare_features(self):
        """Chuẩn bị features cho K-NN model và Region Classifier"""
        try:
            ...
            y_regions = self.df['dialect']  # Labels cho region classification
            ...
            self.is_ready = True
            print(f"Da train K-NN model va Region Classifier voi {len(self.feature_columns)} features")
    ...
    def detect_region(self, features):
        """Phát hiện vùng miền từ thuộc tính audio"""
        try:
            if not self.region_classifier:
                return None
            ...
            # Lấy xác suất cho tất cả các vùng miền
            probabilities = self.region_classifier.predict_proba(feature_vector_scaled)[0]
            region_names = self.region_classifier.classes_
            
            # Tạo dictionary xác suất
            prob_dict = {region_names[i]: probabilities[i] * 100 for i in range(len(region_names))}
    ...
    def find_similar_voices(self, audio_path, k=10, features=None, target_region=None):
        """Tìm K giọng tương tự nhất sử dụng K-NN với lọc theo vùng miền"""
        try:
            ...
            # Lọc dữ liệu theo vùng miền nếu có
            search_df = self.df
            if target_region:
                search_df = self.df[self.df['dialect'] == target_region]
                print(f"Tim kiem trong vung mien: {target_region} ({len(search_df)} files)")
            
            if len(search_df) == 0:
                print(f"Khong co du lieu cho vung mien: {target_region}, su dung tat ca du lieu")
                search_df = self.df
            ...
            # Lấy features từ search_df
            X_search = search_df[self.feature_columns].fillna(0)
            X_search_scaled = self.scaler.transform(X_search)
            
            # Tìm K neighbors gần nhất trong dữ liệu đã lọc
            knn_search = NearestNeighbors(n_neighbors=min(k, len(search_df)), metric='cosine')
            knn_search.fit(X_search_scaled)
            
            distances, indices = knn_search.kneighbors(feature_vector_scaled, n_neighbors=min(k, len(search_df)))
...
def main():
    """Hàm chính"""
    print("ViSpeech - Auto Regional Voice Detection Desktop App")
    print("=" * 60)
    
    root = tk.Tk()
    app = AutoRegionalVoiceApp(root)
