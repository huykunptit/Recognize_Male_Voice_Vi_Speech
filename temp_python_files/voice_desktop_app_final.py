...
class VoiceComparisonApp:
    def __init__(self, root):
        ...
        self.current_region_result = None
    ...
    def create_widgets(self):
        ...
        # Regional detection result
        region_frame = tk.LabelFrame(main_frame, text="Ket Qua Phat Hien Vung Mien", 
                                    font=('Arial', 12, 'bold'), bg='#f0f0f0')
        region_frame.pack(fill='x', pady=10)
        
        region_info_frame = tk.Frame(region_frame, bg='#f0f0f0')
        region_info_frame.pack(fill='x', padx=10, pady=5)
        
        self.region_result_var = tk.StringVar(value="Chua phat hien")
        region_label = tk.Label(region_info_frame, textvariable=self.region_result_var, 
                               font=('Arial', 14, 'bold'), bg='#e8f5e8', fg='#2d5a2d')
        region_label.pack(side='left', fill='x', expand=True, padx=5)
        
        # Dropdown để chọn lại vùng miền nếu cần
        region_control_frame = tk.Frame(region_frame, bg='#f0f0f0')
        region_control_frame.pack(fill='x', padx=10, pady=5)
        
        tk.Label(region_control_frame, text="Neu sai, chon lai:", 
                font=('Arial', 9), bg='#f0f0f0').pack(side='left', padx=5)
        
        self.region_override_var = tk.StringVar(value="")
        region_combo = ttk.Combobox(region_control_frame, textvariable=self.region_override_var,
                                   values=["", "North", "Central", "South"],
                                   state='readonly', width=10)
        region_combo.pack(side='left', padx=5)
        
        apply_region_btn = tk.Button(region_control_frame, text="Ap dung", 
                                     command=self.apply_region_override,
                                     bg='#17a2b8', fg='white', font=('Arial', 9))
        apply_region_btn.pack(side='left', padx=5)
    ...
    def browse_file(self):
        ...
        if filename:
            ...
            # Reset region detection
            self.detected_region = None
            self.current_region_result = None
            self.region_result_var.set("Chua phat hien")
    ...
    def _recording_thread(self, duration, sample_rate):
        ...
        try:
            ...
            # Reset region detection
            self.root.after(0, lambda: setattr(self, 'detected_region', None))
            self.root.after(0, lambda: self.region_result_var.set("Chua phat hien"))
    ...
    def apply_region_override(self):
        """Áp dụng vùng miền được chọn lại từ dropdown"""
        override_region = self.region_override_var.get()
        if not override_region:
            messagebox.showinfo("Thong bao", "Vui long chon vung mien!")
            return
        
        if not self.current_audio_features:
            messagebox.showwarning("Canh bao", "Chua co audio de so sanh! Vui long upload file truoc.")
            return
        
        # Cập nhật detected_region
        self.detected_region = override_region
        self.region_result_var.set(f"Vung mien (override): {override_region}")
        
        # So sánh lại với vùng miền mới
        ...
        if audio_path:
            ...
            messagebox.showwarning("Canh bao", "Chua co file audio!")
    ...
    def _compare_voice_thread(self):
        ...
        try:
            ...
            # Bước 2: Phát hiện vùng miền
            self.root.after(0, lambda: self.progress_var.set("Dang phat hien vung mien..."))
            region_result = self.voice_comparison.detect_region(features)
            
            if region_result:
                self.detected_region = region_result['predicted_region']
                confidence = region_result['confidence']
                self.current_region_result = region_result
                
                # Debug logging
                print(f"\n=== REGIONAL DETECTION DEBUG ===")
                print(f"Predicted Region: {self.detected_region}")
                print(f"Confidence: {confidence:.2f}%")
                print(f"All Probabilities: {region_result.get('probabilities', {})}")
                print(f"Target region for filtering: {self.detected_region}")
                
                # Cập nhật UI với kết quả phát hiện
                region_text = f"Vung mien: {self.detected_region} (Do tin cay: {confidence:.1f}%)"
                self.root.after(0, lambda: self.region_result_var.set(region_text))
            else:
                self.current_region_result = None
                self.detected_region = None
                print("WARNING: Khong the phat hien vung mien!")
            
            # Bước 3: Lưu thuộc tính vào JSON
            self.current_audio_features = features
            self._save_features_to_json(features, region_result)
            
            # Bước 4: Hiển thị thuộc tính
            self.root.after(0, lambda: self._display_audio_features(region_result))
            
            # Bước 5: Thực hiện so sánh với lọc theo vùng miền
            # Sử dụng override region nếu có, nếu không thì dùng detected_region
            target_region_for_comparison = self.detected_region
            if hasattr(self, 'region_override_var') and self.region_override_var.get():
                target_region_for_comparison = self.region_override_var.get()
                print(f"INFO: Su dung override region: {target_region_for_comparison}")
            
            self.root.after(0, lambda: self.progress_var.set("Dang so sanh..."))
            results = self.voice_comparison.find_similar_voices(audio_path, k=10, features=features, target_region=target_region_for_comparison)
            ...
            # Bước 6: Cập nhật UI với kết quả
    ...
    def _save_features_to_json(self, features, region_result=None):
        """Lưu thuộc tính audio và kết quả phát hiện vào file JSON"""
        try:
            ...
            features_data = {
                ...
                "region_confidence": region_result['confidence'] if region_result else None,
                "features": features
            }
    ...
    def _update_results(self, results):
        ...
        self._display_audio_features(self.current_region_result)
    ...
    def _display_audio_features(self, region_result=None):
        ...
        # Hiển thị thông tin vùng miền nếu có
        if region_result:
            features_text += f"Vung mien phat hien: {self.detected_region}\n"
            features_text += f"Do tin cay: {region_result['confidence']:.1f}%\n\n"
...
class VoiceComparisonKNN:
    ...
    def prepare_features(self):
        ...
        try:
            ...
            y_regions = self.df['dialect']
            self.region_classifier = RandomForestClassifier(n_estimators=100, random_state=42)
            self.region_classifier.fit(X_scaled, y_regions)
            
            self.is_ready = True
            print(f"Da train K-NN model voi {len(self.feature_columns)} features")
            print(f"Da train Regional Classifier cho {len(self.regions)} vung mien")
    ...
    def detect_region(self, features):
        """Phát hiện vùng miền từ đặc trưng audio"""
        try:
            if self.region_classifier is None:
                print("ERROR: Region classifier chua duoc train!")
                return None
            ...
            # Predict region
            predicted_region = self.region_classifier.predict(feature_vector_scaled)[0]
            probabilities = self.region_classifier.predict_proba(feature_vector_scaled)[0]
            confidence = max(probabilities) * 100
            
            # Tạo dict probabilities với tên vùng miền
            prob_dict = dict(zip(self.region_classifier.classes_, probabilities))
            ...
            print(f"Confidence: {confidence:.2f}%")
            print(f"Probabilities: North={prob_dict.get('North', 0)*100:.1f}%, Central={prob_dict.get('Central', 0)*100:.1f}%, South={prob_dict.get('South', 0)*100:.1f}%")
            
            return result
        except Exception as e:
            print(f"Loi khi phat hien vung mien: {e}")
            import traceback
            traceback.print_exc()
            return None
    
    def find_similar_voices(self, audio_path, k=10, features=None, target_region=None):
        """Tìm K giọng tương tự nhất sử dụng K-NN, có thể lọc theo vùng miền"""
        try:
            ...
            if target_region:
                ...
                print(f"Available dialects in data: {self.df['dialect'].unique()}")
                
                # Lấy indices của các samples thuộc target_region
                region_mask = self.df['dialect'] == target_region
                region_indices = self.df[region_mask].index.values
                
                print(f"Number of samples in {target_region}: {len(region_indices)}")
                print(f"Total samples: {len(self.df)}")
                
                if len(region_indices) == 0:
                    # Không có dữ liệu cho vùng miền này, trả về toàn bộ
                    print(f"WARNING: Khong co du lieu cho vung mien {target_region}, su dung toan bo du lieu")
                    distances, indices = self.knn_model.kneighbors(feature_vector_scaled, n_neighbors=k)
                    indices = indices[0]
                    distances = distances[0]
                else:
                    # Lọc dữ liệu theo vùng miền
                    X_region = self.df.loc[region_indices, self.feature_columns].fillna(0)
                    X_region_scaled = self.scaler.transform(X_region)
                    
                    # Train K-NN cho vùng miền này
                    knn_region = NearestNeighbors(n_neighbors=min(k, len(X_region_scaled)), metric='cosine')
                    knn_region.fit(X_region_scaled)
                    
                    # Tìm neighbors trong vùng miền
                    n_neighbors_actual = min(k, len(X_region_scaled))
                    distances, local_indices = knn_region.kneighbors(feature_vector_scaled, n_neighbors=n_neighbors_actual)
                    
                    # Map local indices về global indices
                    indices = region_indices[local_indices[0]]
                    distances = distances[0]
                    
                    print(f"After filtering: Found {len(indices)} results from {target_region} region")
            else:
                # Không lọc, sử dụng toàn bộ dữ liệu
                print(f"INFO: Khong loc theo vung mien, su dung toan bo du lieu")
                distances, indices = self.knn_model.kneighbors(feature_vector_scaled, n_neighbors=k)
                indices = indices[0]
                distances = distances[0]
            ...
            # Tạo kết quả
            results = []
            dialect_counts = {}  # Đếm dialect trong kết quả
            ...
            # Debug logging - thống kê kết quả
            print(f"\n=== RESULTS STATISTICS ===")
            print(f"Total results: {len(results)}")
            print(f"Dialect distribution in results: {dialect_counts}")
            if target_region:
                expected_count = dialect_counts.get(target_region, 0)
                print(f"Expected region ({target_region}) in results: {expected_count}/{len(results)} ({100*expected_count/len(results):.1f}%)")
